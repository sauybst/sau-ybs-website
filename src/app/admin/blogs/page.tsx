        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Sistemdeki tüm yazıları, makaleleri ve duyuruları yönetin.</p>
                </div>
                <Link 
            ...
            <div className="flex flex-col gap-4">
                {blogs?.map((blog) => {
                    // Veritabanındaki 'type' sütununa göre yazının türünü belirliyoruz (0: Blog, 1: Makale)
                    const isArticle = blog.type === 1;

                    return (
                        <div key={blog.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md border border-slate-100 transition-all duration-300 p-4 sm:p-5 flex flex-col sm:flex-row gap-5 sm:items-center justify-between group">
                            ...
                            <div className="flex items-center gap-4 sm:gap-6 min-w-0">
                                ...
                                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-slate-50 border border-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden relative">
                                    {blog.cover_image_url ? (
                                        <img src={blog.cover_image_url} alt={blog.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                    ) : (
                                ...
                                <div className="flex flex-col min-w-0">
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-500 font-medium mt-1">
                                        <div className="flex items-center">
                                            <Clock className="flex-shrink-0 mr-1.5 h-4 w-4 text-brand-400" />
                                            <time dateTime={blog.created_at}>
                                                {new Date(blog.created_at).toLocaleDateString('tr-TR', { day: '2-digit', month: 'short', year: 'numeric' })}
                                            </time>
                                        </div>
                                    </div>
                                </div>
                ...
                {(!blogs || blogs.length === 0) && (
                    <div className="flex flex-col items-center justify-center p-12 bg-white border-2 border-dashed border-slate-200 rounded-2xl">
                        ...
                        <h3 className="text-lg font-bold text-slate-900 mb-1">Henüz Yazı Yok</h3>
                        <p className="text-slate-500 text-sm mb-6 text-center max-w-sm">
                            Sistemde kayıtlı herhangi bir blog veya makale bulunamadı. Yeni bir yazı oluşturarak okuyucularınızla buluşun.
                        </p>
                        <Link 
                            href="/admin/blogs/create" 
